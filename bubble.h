...
const int BUBBLE_DEFAULT_VX = 120;
const int BUBBLE_DEFAULT_VY = 0;
const int BUBBLE_DEFAULT_AY = 400;
class Bubble
{
private:
...
public:
    Bubble(double cx, double cy, double r, double vx_=BUBBLE_DEFAULT_VX, double vy_=BUBBLE_DEFAULT_VY, double ay_=BUBBLE_DEFAULT_AY, Color color_=COLOR(0, 0, 255))
    {
        // Bubble constructor
        color = color_;
    ...
    {
        // move the bubble
        double new_x = circle.getX() + vx*t;             //update x coordinate of bubble
        vy = vy + ay*t;                                  //update velocity : v = u + at
        double new_y = circle.getY() + vy*t;             //update y coordinate of bubble
        ...
        if(vy > 0 && new_y > 440 - circle.getRadius()){        //bounce along Y direction at bottom border
